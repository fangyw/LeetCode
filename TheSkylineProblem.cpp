 ...
 * the ith building, respectively, and Hi is its height. It is guaranteed that 0 ≤ 
 * Li, Ri ≤ INT_MAX, 0 , and Ri - Li > 0. You may assume all buildings are perfect 
...
public:
    vector<pair<int,int>> getSkyline(vector<vector<int>> &buildings) {
        // step1: sort the skyline accroding to its index
        vector<pair<int,int>> skyline;

        for(const auto &building : buildings) { // how to identify the left and right
            skyline.emplace_back(building[0], -building[2]);
            skyline.emplace_back(building[1], building[2]);
        }
        ...
        max_bst.insert(0); // dummy node
        int prev = 0, cur = 0;

        for(int i=0; i<skyline.size(); ++i) {
            if (skyline[i].second < 0) { // start
                max_bst.insert(-skyline[i].second);
            } else {
                max_bst.erase(max_bst.find(skyline[i].second));
            }
            // determine whether the edge became a pivot edge
            cur = *max_bst.begin();
            if (cur != prev) {
                res.emplace_back(skyline[i].first, cur);
                prev = cur;
            }
        }
